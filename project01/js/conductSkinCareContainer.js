...
for (let i = 0; i < conductConUlLi.length; i++) {
  arrImgSrc.push(`img/skinCare/skinCare_${i}.jpg`);
  arrImgAlt.push(`skinCare_${i}`);
...
conductConUl.addEventListener('click', e=>{
  ...
  heartImg.forEach((el,idx)=>{
    ...
    if(eTarget === el){
      ...
      if(bool[idx]){
        el.setAttribute('src', `img/skinCare/heart_1.png`)
        el.setAttribute('alt', `mens_1`)
      }else{
        el.setAttribute('src', `img/skinCare/heart_0.png`)
...
const plusBtn = document.querySelector('.plus-con>button');
const liOneClick = document.querySelectorAll('li.oneClick');

plusBtn.addEventListener('click', e=>{

  liOneClick.forEach(el=>{
