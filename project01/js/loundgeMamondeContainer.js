...
for (let i = 0; i < sec1UlLi.length; i++) {
  arrImgSrc.push(`img/loundge/mamonde/mamonde_${i}.jpg`);
  arrImgAlt.push(`mamonde_${i}`);
...
viewGallery.addEventListener('click', e => {
  ...
  for (let i = 0; i < sec1UlLi.length; i++) {
    ...
    //div.market 9개 만들어서 class 넣고 li의 자식으로 넣기
    const marketTag = document.createElement('div');
    marketTag.classList.add('marketTag-click');
    liTag.appendChild(marketTag);


    //img 9개 만들어서 class 넣고 market태그의 자식으로 넣기
    const marketImgTag = document.createElement('img');
    marketImgTag.classList.add('imgTag-click');
    marketTag.appendChild(marketImgTag);


   //src, alt 속성 만들어서 marketImgTag에 넣어주기
    document.createAttribute('src');
    document.createAttribute('alt');
   
    marketImgTag.setAttributeNode(document.createAttribute('src'));
    marketImgTag.setAttributeNode(document.createAttribute('alt'));
   
   
    //arrImg의 각 src, alt 배열 안에 값 넣기
    arrBasketSrc.push(`img/loundge/mamonde/basket.png`);
    arrBasketAlt.push(`market_0`);


    //imgTag에 src, alt 값 넣기
    marketImgTag.setAttribute('src', arrBasketSrc[i]);
    marketImgTag.setAttribute('alt', arrBasketAlt[i]);
...
//    liTag.style.width = '33%';
//    liTag.style.height = 'auto';
//    liTag.style.border = '1px solid  rgba(211, 210, 210, 0.544)';
//    liTag.style.boxSizing = 'border-box';
//    liTag.style.display = 'flex';
//    liTag.style.flexDirection = 'column';
//    liTag.style.justifyContent = 'flex-start';
//    liTag.style.backgroundColor = 'rgb(250, 243, 234)';
...
//    //const imgArr = Array.from(imgTag);
...
//    //이미지에 값 넣는게 안됨.. imgTag가 배열이 아니라서 그런 것 같은데 어떻게 해야하는지
//    //imgTag[i].setAttribute('src', arrImgSrc1[i+10])
//    //imgTag[i].setAttribute('alt', arrImgAlt1[i+10])
